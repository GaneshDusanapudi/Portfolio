...
def create_player_card(slot_index: int) -> Card:
    # Assign random elements for visual variety
    elements = ['fire', 'water', 'leaf', 'air']
    e = elements[slot_index % len(elements)]
    
    attacks = [
        Attack("Strike", dmg=12, element=e),
        Attack("Burst",   dmg=15, element='fire' if e != 'fire' else 'water'),
        Attack("Ult",  dmg=20, element='air'),
    ]
    return Card(owner="player", name=f"Hero {slot_index+1}", hp=80, max_hp=80, attacks=attacks, move_range=3, element=e, index=slot_index)
...
def perform_attack_logic(attacker_c, attacker_r, target_c, target_r, attack_obj):
    # This function runs AFTER the projectile hits
    target = grid.tiles[target_c][target_r].card
    attacker = grid.tiles[attacker_c][attacker_r].card
    
    if target and attacker:
        dmg = attack_obj.dmg + random.randint(-2, 2)
        target.hp -= dmg
        
        # Visuals
        anim_mgr.add_floating_text(f"-{dmg}", *cell_center(target_c, target_r), C_ENEMY if target.owner == "player" else C_HIGHLIGHT)
        
        print(f"{attacker.name} hit {target.name} for {dmg}")
        
        if target.hp <= 0:
            grid.tiles[target_c][target_r].card = None
            anim_mgr.add_floating_text("KO!", *cell_center(target_c, target_r), (255, 0, 0))
